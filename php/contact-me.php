...
if($_POST) {
    ...
    $to_Email = 'matt@thebloxoffice.com'; // Write your email here to receive the form submissions
    $subject = 'New message from The Blox Office Splash Page'; // Write the subject you'll see in your inbox
    ...
    if(!isset($_POST["userChecking"]) || !isset($_POST["userName"]) || !isset($_POST["userEmail"]) || !isset($_POST["userSubject"]) || !isset($_POST["userPhone"]) || !isset($_POST["userMessage"])) {
    ...
    if(strlen($_POST["userPhone"])<6) {
        $output = json_encode(array('type'=>'error', 'text' => '<span><i class="icon ion-close-round"></i></span>Error on 4th field :<br>Please enter a valid phone number.'));
        die($output);
    }
    ...
    if(strlen($_POST["userMessage"])<20) {
        $output = json_encode(array('type'=>'error', 'text' => '<span><i class="icon ion-close-round"></i></span>Error on 5th field :<br>This message is a bit short! Take your time and write a few words.'));
        die($output);
    }
                        ...
                        <tr>
                            <td>
                                ...
                                <div style='margin-top:30px;text-align:center;color:#b3b3b3'>
                                    <p style='font-size:12px;'>2018 The Blox Office®, All Rights Reserved.</p>
                                </div>
                            </td>
                        </tr>
                    </table>
    ...
    if(!$Mailsending) {
        ...
        $output = json_encode(array('type'=>'error', 'text' => '<span><i class="icon ion-close-round"></i></span>Oops! Looks like something went wrong.'));
        die($output);
